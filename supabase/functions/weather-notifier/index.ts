...
async function getUniqueLocationKeys(supabaseAdmin: any): Promise<string[]> {
  ...
  return [
    ...new Set(
      (data as Array<{ location_key: string }>).map((d) => d.location_key)
    ),
  ];
}
...
async function fetchDataForLocation(
  ...
  const [alertsResponse, conditionsResponse] = await Promise.all([
    fetch(alertsUrl),
    fetch(currentConditionsUrl),
  ]);

  const alerts = alertsResponse.ok ? await alertsResponse.json() : [];
  const currentConditions = conditionsResponse.ok
    ? await conditionsResponse.json()
    : [];

  return {
    alerts: Array.isArray(alerts) ? alerts : [],
    currentConditions: Array.isArray(currentConditions)
      ? currentConditions
      : [],
  };
}
...
async function sendAlertNotifications(
  ...
  if (error || !devices?.length) {
    console.warn(
      `Không tìm thấy thiết bị để gửi cảnh báo cho vị trí ${locationKey}`
    );
    return;
  }
  ...
  const alertBody =
    firstAlert.Area?.[0]?.Summary || "Có cảnh báo mới cho khu vực của bạn.";
  const dataPayload = JSON.stringify(alerts);

  const notificationPromises = (devices as Array<{ fcm_token: string }>).map(
    (device) => {
  ...
  console.log(
    `Cảnh báo cho vị trí ${locationKey}: ${successCount}/${devices.length} gửi thành công.`
  );
}
...
async function sendCurrentWeatherNotification(
  ...
  const title = `Current weather conditions (${new Date().toLocaleDateString(
    "vi-VN"
  )})`;
  ...
  const successCount = results.filter((r) => r.status === "fulfilled").length;
  console.log(
    `Daily weather condition fo ${locationKey}: ${successCount}/${devices.length} have sent successfully.`
  );
}
    ...
    for (const locationKey of uniqueLocationKeys) {
      try {
        ...
        if (currentConditions.length > 0) {
          ...
          await sendCurrentWeatherNotification(
            locationKey,
            currentConditions[0],
            accessToken,
            serviceAccount.project_id,
